<?php
// Use absolute paths to avoid path issues when included from different locations
...
if ($checkColumnStmt->rowCount() == 0) {
    ...
    if ($alterTableStmt->execute()) {
        echo "Added uuid column to users table.<br>";
        ...
        echo "Updated $updateCount existing users with UUIDs.<br>";
        ...
        if ($addUniqueStmt->execute()) {
            echo "Added unique constraint to uuid column.<br>";
        } else {
            echo "Failed to add unique constraint to uuid column.<br>";
        }
    } else {
        echo "Failed to add uuid column to users table.<br>";
    }
} else {
    echo "UUID column already exists in users table.<br>";
}
...
if ($checkStatusStmt->rowCount() == 0) {
    ...
    if ($addStatusStmt->execute()) {
        ...
        echo "Failed to add status column to users table.<br>";
    }
} else {
